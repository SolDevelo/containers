...
prev_upstream_sha_for_path() {
  ...
  if [[ -z "$prev_sha" && "$HISTORY_DEEPENED" -eq 0 ]]; then
    ...
    git fetch "${BITNAMI_REMOTE}" main --deepen=2000 --quiet || true
    HISTORY_DEEPENED=1
    prev_sha=$(git log --oneline -n 2 \
      "remotes/${BITNAMI_REMOTE}/main" \
      -- "$path" 2>/dev/null | awk 'NR==2{print $1}')
...
for local_dir in "${CHANGED[@]}"; do
  ...
  if [[ -s "$ancestor_df" ]]; then
    ...
    # No ancestor found in shallow history. Prefer full upstream Dockerfile body
    # (preserving our header + selected labels) to avoid missing upstream changes
    # such as component version bumps spread across older commits.
    result_df=$(mktemp)
