  ...
  while IFS= read -r upstream_file; do
    ...
    if [[ "$our_functional" == "$upstream_functional" ]]; then
      ...
      if [[ -s "$ancestor_stripped_f" ]]; then
        ...
        # No usable ancestor commit found in shallow history
        MANUAL_REVIEW+=("${local_dir}/${rel}")
        echo "     ! no ancestor found; kept ours (functional diff): $rel"
      fi
...
# depth=50 gives enough history to find the previous upstream version of any recently-changed
# file, which is required for the 3-way merge below.
git fetch "${BITNAMI_REMOTE}" main --depth=50 --quiet
echo ""
...
for local_dir in "${CHANGED[@]}"; do
  ...
  if [[ -s "$ancestor_df" ]]; then
    ...
    # No ancestor found in shallow history: take upstream body with our header + labels
    result_df=$(mktemp)
    python3 -c "$MERGE_FILE_PY" "${local_dir}/Dockerfile" "$upstream_df" "dockerfile" \
      > "$result_df"
    mv "$result_df" "${local_dir}/Dockerfile"
    echo "     ~ merged (no ancestor available, took upstream body): Dockerfile"
  fi
